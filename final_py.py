    ...
    def transform(self, X):
        ...
        for column in self.columns_to_cap:
            if column != 'date' and X[column].dtype != 'object':
                lower_threshold = X[column].quantile(0.05)
                upper_threshold = X[column].quantile(0.95)
...
features = train_data.columns.difference(['log_bike_count'])
...
raw_predictions = model_pipeline.predict(X_test)

prediction_transformer = CustomPredictionTransformer()
predictions = prediction_transformer.transform(raw_predictions)

result_df = pd.DataFrame({'log_bike_count': predictions})
